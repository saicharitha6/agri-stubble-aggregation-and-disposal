<?php
  ...
  if(!empty($username) || !empty($password) || !empty($Gender) || !empty($email) || !empty($phone))
  {
    ...
    if(mysqli_connect_error())
    {
      ...
      $INSERT = "INSERT Into seller (username,password,Gender,email,phone) values(?, ?, ?, ?, ?)";
      ...
      if($rnum==0)
      {
        ...
        $relogin= "<script>  alert('someone already register using this email'); window.location ='seller_Registration.html';</script>";
        echo $relogin;
      }
      $stmt ->close();
      $conn ->close();
